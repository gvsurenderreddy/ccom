...
$auth = canRegisteredUserAccess("modcontacts");
...
$page->addVar('headers', 'titulopagina', $trans->_('Gestion contactos'));
$page->addVar('page', 'labelalta', $trans->_("Alta contacto") );
...
switch($modo){
	case "filtrar-elemento":
		...
		$extracondicion  = " AND contact_name.name LIKE '%$filtranombre_s%' ";
...
if ($mostrarEdicion){
	...
	$page->setAttribute( 'edicion', 'src', 'edicion_contacto.txt' );
...
if ($mostrarListado){
	...
	$page->setAttribute( 'listado', 'src', 'listado_contactos.txt' );
	...
	$sql = "SELECT * FROM contacts
		WHERE id_contact>0 $extracondicion
		ORDER BY contact_name  ASC LIMIT $min,$maxfilas";
	$res = query($sql);
