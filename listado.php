...
//$acceso = negociar_Autorizacion("p8");
...
if ($res){
	...
	if ($row){
		...
		if (!$esVertical) {
			...
			$genListado .= Datos2Codigo_Vertical($row);			
		}
		...
		if ($nombrePorcentaje){
	 	...
	 	$genListado = "<tr class='lineadatos'>
				<td class='dato'> <div style='text-align:center;font-weight: bold;'>"."El listado resulto vacio"."</div> </td></tr>";	
	 	$genTotales = "<tr class='head'>
...
<table style="background-color: #fefefe" width='<?php echo ($esVertical)?"100":"100%"; ?>'>
<?php
	echo $genCabecera;
	echo $genListado;
	echo $genTotales;
...
function Datos2Codigo_Vertical( $datos,$par=false ){
	...
	foreach ($datos as $key=>$value){
		...
		if (!esNumero2($key)) {
			...
			$out .= "\t<tr><td  class='headitem' width='10'><nobr>$titulo:</nobr></td><td class='dato'>$value &nbsp;</td></tr>\n";
		}
...
function PostProcesarSQL( $cod ) {
	...
	$cod = str_replace("%ESTADO%",		CleanText($_GET["Estado"]),$cod);
	$cod = str_replace("%CODCLIENTE%",		CleanText($_GET["CodCliente"]),$cod);
	...
	$cod = str_replace("%IDDELEGACION%",	CleanID($_GET["IdDelegacion"]),$cod);
