...
/**
 * Ayudas para escribir codigo que genera o trata con js
 *
 * @package ecomm-aux
 */
...
//$etiqueta_s = JSEncodeString(iconv("iso-8859-1","UTF-8",$row["etiqueta"])) ;   

function JSEncodeString( $var ){
         $cadenaDeEntrada = $var;
                    $ascii = '';
        ...
        $cadenaSalida = sprintf('%s', $ascii);
        if ($cadenaDeEntrada and !$cadenaSalida)
          return addslashes( $cadenaDeEntrada );
        return $cadenaSalida;        
}                    
...
function jsLabel($var, $texto, $reformatear=true){
	global $entHtml, $entXML;
	
	if ($reformatear){ //escapa cosas raras a codigos compatibles con xml 
		$texto = htmlentities($texto,ENT_QUOTES,'UTF-8');		
		$texto = str_replace($entHtml, $entXML, $texto);
	}
	$texto = addslashes($texto);	
	return "var po_$var=\"$texto\";\n"; 		
} 
...
function jsSetup($key,$value){	
	return "var cfg_$key = '$value';\n";	
} 
...
class jsLextable {
	...
	function jsDump($prefix = "L",$modo="html",$conDefinicionVariable=true, $toUTF8=false) {
		global $entHtml, $entXML;
		
		$out = "";
		
		if ($conDefinicionVariable)
			$out = "var $prefix = new Array();\n";
		...
		foreach ($this->lex as $key=>$value){
			...
			if ( $toUTF8){
                             ...
                             $keyformat = JSEncodeString(iconv("iso-8859-1","UTF-8",$key));                               
                        } else {
                             $keyformat = JSEncodeString($key); 
                             if (!$keyformat and $key){
                                   $keyformat = addslashes($key);
                             }
